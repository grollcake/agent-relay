...
Do not use this file for progress tracking, current task status, temporary plans, or next steps. Use `memento.log` and `.memento/runs/` artifacts for that.
...
- Planner and Executor notify Director when artifacts are complete and provide a
  suggested event summary; only Director writes `memento.log`.
- The `memento` binary is Director-owned; delegates must not use its
