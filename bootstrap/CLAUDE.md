...
4. Use only the `LeadAI`, `PlanAI`, and `ExecAI` roles, and route PlanAI/ExecAI communication through LeadAI; LeadAI delegates in the background, reads only the `LeadAI Brief` by default before ExecAI delegation, keeps delegation prompts brief by referring to source paths instead of restating their contents, immediately returns control to the user with a short status, remains available for new user requests, and must not block on polling, sleep, or delegated work.
