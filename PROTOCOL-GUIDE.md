...
Agent Relay의 핵심은 세 가지 습관입니다.

1. 합류할 때 읽는다.
2. 의미 있는 작업을 마치면 기록한다.
3. 다음 에이전트가 바로 이어받기 어려울 때만 handoff를 남긴다.

영문 프로토콜에서는 이 세 가지를 다음 이름으로 부릅니다.

| 원칙 | 의미 |
|---|---|
| `Read when joining` | 새 세션, 새 에이전트, 재개 상황에서는 최근 맥락을 먼저 읽습니다. |
| `Record when finishing` | 파일 변경, 테스트, 설계 판단처럼 다음 작업자에게 의미 있는 결과를 `relay.log`에 남깁니다. |
| `Handoff only when needed` | 로그 한 줄로 부족한 경우에만 별도 handoff 문서를 작성합니다. |
...
같은 세션에서는 `agent=`, `from=`, `to=`에 들어가는 에이전트 이름을 일관되게 유지합니다.
...
다음 에이전트가 이미 명확할 때만 `to-<target-agent>`를 선택적으로 붙입니다.
...
5. `AGENTS.md`가 이미 있으면 기존 내용을 보존하고 Agent Relay 섹션만 병합합니다.
