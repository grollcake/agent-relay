...
func newHarness(t *testing.T) *testHarness {
	...
	project := t.TempDir()
	relayDir := filepath.Join(project, ".agent-relay")
	if err := copyTree(filepath.Join(root, "bootstrap", ".agent-relay"), relayDir, map[string]bool{"scripts": true, "bin": true}); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.MkdirAll(filepath.Join(relayDir, "bin"), 0o755); err != nil {
		t.Fatal(err)
	}
	binary := filepath.Join(relayDir, "bin", binaryName(runtime.GOOS))
	...
	if err := os.WriteFile(filepath.Join(relayDir, "bin", "SHA256SUMS"), []byte(checksum), 0o644); err != nil {
		t.Fatal(err)
	}
	writeLog(t, relayDir,
	...
	harness := &testHarness{}
	harness.app = New(project, relayDir)
...
func writeLog(t *testing.T, relayDir string, lines ...string) {
	t.Helper()
	content := strings.Join(lines, "\n")
	if content != "" {
		content += "\n"
	}
	if err := os.WriteFile(filepath.Join(relayDir, "relay.log"), []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}
}
