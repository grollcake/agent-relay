...
func parseRecord(line string, lineNumber int) (Record, error) {
	...
	if len(fields) < 4 || len(fields) > 6 {
		return Record{}, fmt.Errorf("malformed relay log line %d", lineNumber)
	}
	record := Record{
...
func (a *App) readRecords() ([]Record, error) {
	file, err := os.Open(a.relayPath("relay.log"))
	if err != nil {
		return nil, err
	}
	defer file.Close()
...
func (a *App) appendRecord(record Record) (string, error) {
	...
	file, err := os.OpenFile(a.relayPath("relay.log"), os.O_APPEND|os.O_WRONLY, 0)
	if err != nil {
		return "", err
	}
	defer file.Close()
...
func (a *App) runNewRound(args []string) error {
	...
	matches, err := filepath.Glob(a.relayPath("runs", key+"-*"))
	if err != nil {
		return err
	}
	if len(matches) > 0 {
	...
	_ = line
	fmt.Fprintf(a.Stdout, "task_id='%s'\nkey='%s'\nplan_path='.agent-relay/runs/%s-PLAN.md'\nrun_path_template='.agent-relay/runs/%s-RUN-NN.md'\nreview_path_template='.agent-relay/runs/%s-REVIEW-NN.md'\nclose_path='.agent-relay/runs/%s-CLOSE.md'\n", taskID, key, key, key, key, key)
	return nil
}
