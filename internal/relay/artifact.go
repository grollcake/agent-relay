...
func artifactNameMatches(event, path string) bool {
	...
	switch event {
	case eventPlanned:
		pattern = `^\.agent-relay/runs/.+-PLAN\.md$`
	case eventExecuted:
		pattern = `^\.agent-relay/runs/.+-RUN-[0-9]{2}\.md$`
	case eventReview:
		pattern = `^\.agent-relay/runs/.+-REVIEW-[0-9]{2}\.md$`
	case eventClose:
		pattern = `^\.agent-relay/runs/.+-CLOSE\.md$`
	default:
		return false
	}
...
func (a *App) CheckArtifact(event, path, expectedTaskID string) error {
	...
	if !strings.HasPrefix(path, ".agent-relay/runs/") {
		return fmt.Errorf("artifact-check: %s path must be under .agent-relay/runs/: %s", event, path)
	}
...
func (a *App) runCheckArtifact(args []string) error {
	if len(args) < 2 || len(args) > 3 {
		return errors.New("usage: agent-relay check-artifact <PLANNED|EXECUTED|REVIEW|CLOSE> <path> [task-id]")
	}
	taskID := ""
	if len(args) == 3 {
		taskID = args[2]
