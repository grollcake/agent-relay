...
func TestNewRoundCollisionBranchAndRandomErrors(t *testing.T) {
	t.Run("artifact key collision", func(t *testing.T) {
		...
		collision := harness.app.relayPath("runs", "20260711-1001-collision-PLAN.md")
...
func TestAgentBlockLintBranches(t *testing.T) {
	cases := []struct {
		...
		{"matching", "<agent-relay-rules>same</agent-relay-rules>", "<agent-relay-rules>same</agent-relay-rules>", 0},
		{"agents missing", "plain", "<agent-relay-rules>same</agent-relay-rules>", 1},
		{"claude missing", "<agent-relay-rules>same</agent-relay-rules>", "plain", 1},
		{"different", "<agent-relay-rules>one</agent-relay-rules>", "<agent-relay-rules>two</agent-relay-rules>", 1},
	}
...
func TestUpdateAndMergeErrorBranches(t *testing.T) {
	...
	upstream := t.TempDir()
	if err := os.MkdirAll(filepath.Join(upstream, "bootstrap", ".agent-relay"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Remove(harness.app.relayPath("VERSION")); err != nil {
		t.Fatal(err)
	}
	if err := harness.fail("update", "--upstream", upstream); err == nil {
	...
	if err := harness.fail("merge-agent-block", filepath.Join(harness.app.ProjectDir, "target.md"), invalid); err == nil {
		t.Fatal("merge accepted a source without an Agent Relay block")
	}
}
