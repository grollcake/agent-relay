...
func TestDiscoverFromEnvironmentAndWorkingDirectory(t *testing.T) {
	...
	t.Setenv("AGENT_RELAY_DIR", harness.app.RelayDir)
	discovered, err := Discover()
	if err != nil {
		t.Fatal(err)
	}
	if !samePath(discovered.RelayDir, harness.app.RelayDir) {
		t.Fatalf("discovered %s, want %s", discovered.RelayDir, harness.app.RelayDir)
	}

	t.Setenv("AGENT_RELAY_DIR", "")
...
func TestRunHelpUnknownAndArgumentErrors(t *testing.T) {
	...
	for _, args := range [][]string{{"help"}, {"--help"}, {"-h"}} {
		...
		if !strings.Contains(output, "agent-relay append") {
	...
	for _, args := range [][]string{
		...
		if err := harness.fail(args...); err == nil {
			t.Fatalf("agent-relay %v unexpectedly passed", args)
		}
	}
	if binaryName("windows") != "agent-relay.exe" || binaryName("linux") != "agent-relay" {
...
func TestCheckArtifactCommandAndValidationErrors(t *testing.T) {
	harness := newHarness(t)
	path := ".agent-relay/runs/20260711-1000-check-PLAN.md"
	...
	cases := []struct {
		...
		{"unsafe path", eventPlanned, ".agent-relay/runs/../PLAN.md", "", ""},
		{"wrong name", eventPlanned, ".agent-relay/runs/check-RUN-01.md", "", ""},
		{"missing file", eventPlanned, ".agent-relay/runs/missing-PLAN.md", "", ""},
		{"placeholder", eventPlanned, ".agent-relay/runs/placeholder-PLAN.md", "<todo>", ""},
		{"run todo", eventExecuted, ".agent-relay/runs/todo-RUN-01.md", validRun("abcd", "01") + "TODO\n", "abcd"},
		{"task mismatch", eventPlanned, ".agent-relay/runs/task-PLAN.md", validPlan("wrong"), "abcd"},
		{"invalid date", eventPlanned, ".agent-relay/runs/date-PLAN.md", strings.Replace(validPlan("abcd"), "2026-07-11", "11/07/2026", 1), "abcd"},
		{"run status", eventExecuted, ".agent-relay/runs/status-RUN-01.md", strings.Replace(validRun("abcd", "01"), "Status: complete", "Status: checkpoint", 1), "abcd"},
		{"review result", eventReview, ".agent-relay/runs/result-REVIEW-01.md", strings.Replace(validReview("abcd", "01"), "ready-for-user-decision", "unknown", 1), "abcd"},
	}
