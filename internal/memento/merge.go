...
func MergeAgentBlock(target, source string) error {
	...
	sourceBlock := mementoBlockPattern.Find(sourceContent)
	if len(sourceBlock) == 0 {
		return errors.New("source missing <memento-rules> block")
	}
	...
	location := mementoBlockPattern.FindIndex(targetContent)
	var merged []byte
	if location != nil {
...
func (a *App) runMergeAgentBlock(args []string) error {
	if len(args) != 2 {
		return errors.New("usage: memento merge-agent-block <target-file> <source-file>")
	}
	...
	fmt.Fprintf(a.Stdout, "merged Memento AI block: %s\n", args[0])
	return nil
}
