...
func TestTemplatesAreRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{eventPlanned, "plan.md", ".memento/runs/20260711-1000-probe-PLAN.md"},
		{eventExecuted, "run.md", ".memento/runs/20260711-1000-probe-RUN-01.md"},
		{eventReview, "review.md", ".memento/runs/20260711-1000-probe-REVIEW-01.md"},
		{eventClose, "close.md", ".memento/runs/20260711-1000-probe-CLOSE.md"},
	}
	for _, testCase := range cases {
		t.Run(testCase.event, func(t *testing.T) {
			target := harness.app.projectPath(testCase.path)
			mustCopy(t, harness.app.mementoPath("templates", testCase.template), target, 0o644)
...
func TestTransitionMatrixAndAllowedFlows(t *testing.T) {
	...
	for _, prior := range priors {
		for _, next := range events {
			...
			writeLog(t, harness.app.MementoDir, lines...)
	...
	appendRound := func(round, result string) {
		runPath := ".memento/runs/" + key + "-RUN-" + round + ".md"
		reviewPath := ".memento/runs/" + key + "-REVIEW-" + round + ".md"
	...
	closePath := ".memento/runs/" + key + "-CLOSE.md"
...
func TestArtifactConsistency(t *testing.T) {
	...
	wrongRun := ".memento/runs/20260711-1000-wrong-key-RUN-01.md"
	...
	wrongReview := ".memento/runs/" + key + "-REVIEW-02.md"
	...
	cleanLog := readFile(t, harness.app.mementoPath("memento.log"))
	log := strings.ReplaceAll(string(cleanLog), runPath, wrongRun)
	if err := os.WriteFile(harness.app.mementoPath("memento.log"), []byte(log), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := harness.fail("lint"); err == nil {
		t.Fatal("lint accepted an artifact key mismatch")
	}
	if err := os.WriteFile(harness.app.mementoPath("memento.log"), cleanLog, 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMergeAgentBlock(t *testing.T) {
	...
	if !strings.Contains(string(readFile(t, appendTarget)), "<memento-rules>") {
